...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '💎 Potential Winnings', value: `${(playAmount * game.currentMultiplier).toFixed(2)} VEX (~$${(playAmount * game.currentMultiplier * Economics.getCurrentVEXPrice()).toFixed(2)})`, inline: true },
    ...
    async handleCashOut(interaction, gameId) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                { name: '💰 Play Amount', value: `${game.playAmount.toFixed(2)} VEX (~$${(game.playAmount * Economics.getCurrentVEXPrice()).toFixed(2)})`, inline: true },
                ...
                { name: '💸 House Edge', value: `${burnAmount.toFixed(2)} VEX (~$${(burnAmount * Economics.getCurrentVEXPrice()).toFixed(2)})`, inline: true },
                { name: '📊 Net Profit', value: `${(winnings - game.playAmount - burnAmount).toFixed(2)} VEX (~$${((winnings - game.playAmount - burnAmount) * Economics.getCurrentVEXPrice()).toFixed(2)})`, inline: true },
